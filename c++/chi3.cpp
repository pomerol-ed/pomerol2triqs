...
namespace pomerol2triqs {
  ...
  auto pomerol_ed::compute_chi3(gf_struct_t const &gf_struct, Mesh const &mesh, block_order_t block_order, channel_t channel,
            ...
            for (int b : range(A_size))
              for (int c : range(B_size))
                for (int d : range(B_size)) {
                  ...
                  auto chi3_el =
                     block_order == AABB ? slice_target_to_scalar(chi3_block, a, b, c, d) : slice_target_to_scalar(chi3_block, a, d, c, b);

                  Pomerol::ParticleIndex pom_CX1_i = lookup_pomerol_index({A, a});
                  Pomerol::ParticleIndex pom_C2_i  = block_order == AABB ? lookup_pomerol_index({A, b}) : lookup_pomerol_index({B, d});
                  Pomerol::ParticleIndex pom_CX3_i = lookup_pomerol_index({B, c});
                  Pomerol::ParticleIndex pom_C4_i  = block_order == AABB ? lookup_pomerol_index({B, d}) : lookup_pomerol_index({A, b});

                  Pomerol::Channel pom_channel = pom_channels[int(channel)];

                  Pomerol::ThreePointSusceptibility pom_chi3(pom_channel, *states_class, *matrix_h, ops_container->getCreationOperator(pom_CX1_i),
                                                             ops_container->getAnnihilationOperator(pom_C2_i),
                                                             ops_container->getCreationOperator(pom_CX3_i),
                                                             ops_container->getAnnihilationOperator(pom_C4_i), *rho);
  ...
  auto pomerol_ed::chi3_iw_inu(chi3_iw_inu_params_t const &p) -> block2_gf<w_nu_t, tensor_valued<4>> {
    ...
    auto chi3 = compute_chi3<w_nu_t>(p.gf_struct, mesh_bf, p.block_order, p.channel, p.blocks, filler, p.pole_res, p.coeff_tol);
