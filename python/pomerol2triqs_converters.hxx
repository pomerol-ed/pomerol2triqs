...
// --- C++ Python converter for g2_parameters_t
...
template <> struct py_converter<g2_parameters_t> {
 static PyObject *c2py(g2_parameters_t const & x) {
 ...
 static g2_parameters_t py2c(PyObject *dic) {
  g2_parameters_t res;
 ...
 _error:
   fs2 << "\n---- There " << (err > 1 ? "are " : "is ") << err<< " error"<<(err >1 ?"s" : "")<< " in Python -> C++ transcription for the class g2_parameters_t\n" <<fs.str();
